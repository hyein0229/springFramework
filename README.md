# springFramework
## Content
### IoC Container
- [Spring IoC Containter 와 Bean](IoC/0.Introduction.md)
- [Container](IoC/1.Container.md)
- [Bean](IoC/2.Bean.md)
- [Singleton](IoC/3.Singleton.md)
- [Dependency Injection](IoC/4.DI.md)
- [Lazy-initialized Beans](IoC/5.Lazy-initialized.md)
- [Method Injection](IoC/6.Method-Injection.md)
- [BeanScopes](IoC/7.BeanScopes.md)

### Transaction
- [Spring Transaction Management](transaction/0.Transaction_Management.md)
- [Declarative Transaction Management(선언적 트랜잭션 관리)](transaction/1.Declarative_Transaction_Management.md)
- [Transaction Abstraction(Spring 의 트랜잭션 추상화)](transaction/2.Transaction_Abstraction.md)
- [Synchronizing Resources with Transactions](transaction/3.Synchronizing_resources_with_transaction.md)
- [@Transactional](transaction/4.Using_Transactional.md)
- [Transaction Propagation](transaction/5.Propagation.md)
- [Programmatic Transaction Management](transaction/6.Programmatic_Transaction_Management.md)

### AOP
- [리플렉션(Reflection)](aop/0.Reflection.md)
- [동적 프록시 기술(DynamicProxy)](aop/1-1.DynamicProxy_ProxyingMechanisms.md)
- [프록시팩토리(ProxyFactory)](aop/1-2.ProxyFactory.md)
- [@Aspect](aop/3.Aspect.md)
- [AOP Concepts](aop/4.AOP_Concepts.md)
- [AOP Advice](aop/5.AOP_Advice.md)
- [AOP 포인트컷(Pointcut)](aop/6.AOP_Pointcut.md)
- [프록시 내부 호출 문제와 CGLIB](aop/7.AOP_ProxyInternalCall_And_CGLIB.md)

### 디자인 패턴(Design Pattern)
- [템플릿 메서드 패턴(TemplateMethodPattern)](design-patterns/0.TemplateMethodPattern.md)
- [전략 패턴(StrategyPattern)](design-patterns/1.StrategyPattern.md)
- [템플릿 콜백 패턴(TemplateCallBackPattern)](design-patterns/2.TemplateCallBackPattern.md)
- [프록시 패턴(ProxyPattern)](design-patterns/3.ProxyPattern.md)
- [데코레이터 패턴(DecoratorPattern)](design-patterns/4.DecoratorPattern.md)

### 동시성 처리(Concurrency)
- [동시성(concurrency)](concurrency/0.Concurrency.md)
- [synchronized](concurrency/1.synchronized.md)
- [DatabaseLock(PessimisticLock, OptimisticLock, NamedLock)](concurrency/2.DatabaseLock.md)
- [분산락(DistributedLock)](concurrency/3.DistributedLock.md)

## 참고 자료
- [김영한 스프링 핵심 원리 - 기본편](https://www.inflearn.com/course/%EC%8A%A4%ED%94%84%EB%A7%81-%ED%95%B5%EC%8B%AC-%EC%9B%90%EB%A6%AC-%EA%B8%B0%EB%B3%B8%ED%8E%B8)
- [김영한 스프링 핵심 원리 - 고급편](https://www.inflearn.com/course/%EC%8A%A4%ED%94%84%EB%A7%81-%ED%95%B5%EC%8B%AC-%EC%9B%90%EB%A6%AC-%EA%B3%A0%EA%B8%89%ED%8E%B8)
- [Spring Framework Documentation](https://docs.spring.io/spring-framework/reference/index.html) 



